...
const RegisterBox = (props) => {
  const [userLineColor, setUserLineColor] = React.useState(
    adSimplifierTheme.colors.black10
  );
  const [passLineColor, setPassLineColor] = React.useState(
    adSimplifierTheme.colors.black10
  );
  const [emailLineColor, setEmailLineColor] = React.useState(
    adSimplifierTheme.colors.black10
  );
  function handleRegister(e) {
    ...
    if (
      ...
      if (props.username.length < 5 || props.username !== '')
        setUserLineColor(adSimplifierTheme.colors.englishVermillion);
    }
  }
  return (
      ...
      <S.MarginContainer>
        ...
        <GS.FormContainer height={'80%'}>
          <form onSubmit={handleRegister} autoComplete={'off'}>
            ...
            <Input
              labelValue={'Email'}
              placeholderValue={'usuario@email.com'}
              type={'email'}
              icon={emailIcon}
              inputId={'email'}
              setChange={props.setEmail}
            />
